...
# Method: detect_duplicates for data.table, character ID, Search_Strategy, and numeric threshold
#------------------------------------------------------------------------------
method(
  detect_duplicates,
  ...
  edges <- scored[, .(from = get(id), to = get(id2))]
