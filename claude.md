...
- **`match_features()`** — exported verb (M2). Builds a wide one-row-per-pair feature `data.table` from a joinery match result. Dispatches on strategy class: `Search_Strategy` returns the full token schema (`searched`, `found`, `match_id`, `stage`, `score`, `cnt`, `icnt`, `ipos`, `stage_*`, `scnt`, `rcnt`, `r1..rn`, `m_<col>_*`, `f_<col>_*`, `s_<col>_*`); `Embedding_Strategy` returns the reduced schema (core columns only — string-sim and embedding norms land in M3). Column order is locked at end of M2 and is the public API of v0.7 — additions only, never reorder or rename. Lives in `R/match_features.R`; schema in `notes/calibration_design.md` §6.
